/* Target-dependent code for the TLCS900.
...
#include "tlcs900-tdep.h"
#include "features/tlcs900.c"
...
struct gdbarch_tdep
{
  /* Number of bytes used for address:
      2 bytes for all TLCS900 family
      3 bytes for eTLCS900 CPUs operating in ADL mode */
  int addr_length;
...
tlcs900_get_insn_info (struct gdbarch *gdbarch, const gdb_byte *buf, int *size);

static const char *tlcs900_reg_names[] =
{
  /* 24 bit on eTLCS900, else 16 bit */
  "af", "bc", "de", "hl",
  "sp", "pc", "ix", "iy",
  "af'", "bc'", "de'", "hl'",
  "ir",
...
static const char *
tlcs900_register_name (struct gdbarch *gdbarch, int regnum)
{
  if (regnum >= 0 && regnum < ARRAY_SIZE (tlcs900_reg_names))
    return tlcs900_reg_names[regnum];
...
/* Return the type of a register specified by the architecture.  Only
   ...
   use "register_type".  */
static struct type *
tlcs900_register_type (struct gdbarch *gdbarch, int reg_nr)
{
...
static int
tlcs900_is_pop_rr (const gdb_byte buf[], int *size)
{
  switch (buf[0])
    {
    case 0xc1:
      *size = 1;
      return TLCS900_BC_REGNUM | 0x100;
    case 0xd1:
      *size = 1;
      return TLCS900_DE_REGNUM | 0x100;
    case 0xe1:
      *size = 1;
      return TLCS900_HL_REGNUM | 0x100;
    case 0xf1:
      *size = 1;
      return TLCS900_AF_REGNUM | 0x100;
    case 0xdd:
      *size = 2;
      return (buf[1] == 0xe1) ? (TLCS900_IX_REGNUM | 0x100) : 0;
    case 0xfd:
      *size = 2;
      return (buf[1] == 0xe1) ? (TLCS900_IY_REGNUM | 0x100) : 0;
    }
  *size = 0;
  return 0;
}
...
static int
tlcs900_is_push_rr (const gdb_byte buf[], int *size)
{
  switch (buf[0])
    {
    case 0xc5:
    ...
    case 0xdd:
      *size = 2;
      return (buf[1] == 0xe5) ? (TLCS900_IX_REGNUM | 0x100) : 0;
    case 0xfd:
      *size = 2;
      return (buf[1] == 0xe5) ? (TLCS900_IY_REGNUM | 0x100) : 0;
    }
  *size = 0;
  return 0;
}

/* Function: tlcs900_scan_prologue

   This function decodes a function prologue to determine:
     1) the size of the stack frame
     2) which registers are saved on it
     3) the offsets of saved regs
   This information is stored in the tlcs900_unwind_cache structure.
   ...
   SDCC function prologue may have up to 3 sections (all are optional):
	...
	add	iy, sp
	ld	sp, iy
       e) same as c, but for eTLCS900
	lea	hl, ix - #nn
	ld	sp, hl
       f) same as d, but for eTLCS900
	lea	iy, ix - #nn
	ld	sp, iy
*/

static int
tlcs900_scan_prologue (struct gdbarch *gdbarch, CORE_ADDR pc_beg, CORE_ADDR pc_end,
		   struct tlcs900_unwind_cache *info)
{
  ...
  if ((reg = tlcs900_is_pop_rr(prologue, &pos)))
    {
      int i;
      int size = pos;
      gdb_byte regs[8]; /* TLCS900 have only 6 register pairs */
      regs[0] = reg & 0xff;
      for (i = 1; i < 8 && (regs[i] = tlcs900_is_pop_rr (&prologue[pos], &size));
	   ++i, pos += size);
      /* now we expect series of PUSHs in reverse order */
      for (--i; i >= 0 && regs[i] == tlcs900_is_push_rr (&prologue[pos], &size);
	   --i, pos += size);
      if (i == -1 && pos > 0)
  ...
  if (info->prologue_type.interrupt)
    {
      info->saved_regs[TLCS900_AF_REGNUM].set_addr (len++);
      info->saved_regs[TLCS900_BC_REGNUM].set_addr (len++);
      info->saved_regs[TLCS900_DE_REGNUM].set_addr (len++);
      info->saved_regs[TLCS900_HL_REGNUM].set_addr (len++);
      info->saved_regs[TLCS900_IY_REGNUM].set_addr (len++);
    }
  ...
  if (info->prologue_type.fp_sdcc)
    info->saved_regs[TLCS900_IX_REGNUM].set_addr (len++);
...
static CORE_ADDR
tlcs900_skip_prologue (struct gdbarch *gdbarch, CORE_ADDR pc)
{
  ...
  {
    struct tlcs900_unwind_cache info = {0};
    struct trad_frame_saved_reg saved_regs[TLCS900_NUM_REGS];
    ...
    prologue_end = tlcs900_scan_prologue (gdbarch, func_addr, func_end, &info);
...
tlcs900_return_value (struct gdbarch *gdbarch, struct value *function,
  ...
  if (writebuf != NULL)
    {
      if (len > 2)
	{
	  regcache->cooked_write_part (TLCS900_DE_REGNUM, 0, len - 2, writebuf+2);
	  len = 2;
	}
      regcache->cooked_write_part (TLCS900_HL_REGNUM, 0, len, writebuf);
    }
  ...
  if (readbuf != NULL)
    {
      if (len > 2)
	{
	  regcache->cooked_read_part (TLCS900_DE_REGNUM, 0, len - 2, readbuf+2);
	  len = 2;
	}
      regcache->cooked_read_part (TLCS900_HL_REGNUM, 0, len, readbuf);
    }
...
static struct tlcs900_unwind_cache *
tlcs900_frame_unwind_cache (struct frame_info *this_frame,
			void **this_prologue_cache)
{
  ...
  if (*this_prologue_cache)
    return (struct tlcs900_unwind_cache *) *this_prologue_cache;

  info = FRAME_OBSTACK_ZALLOC (struct tlcs900_unwind_cache);
  ...
  *this_prologue_cache = info;
  ...
  if ((start_pc > 0) && (start_pc <= current_pc))
    tlcs900_scan_prologue (get_frame_arch (this_frame),
  ...
  if (info->prologue_type.fp_sdcc)
    {
      ...
      this_base = get_frame_register_unsigned (this_frame, TLCS900_IX_REGNUM);
  ...
  else
    {
	 ...
	 stack space added back.  */
      this_base = get_frame_register_unsigned (this_frame, TLCS900_SP_REGNUM);
  ...
  info->saved_regs[TLCS900_PC_REGNUM].set_addr (info->prev_sp);

  /* The previous frame's SP needed to be computed.  Save the computed
     value.  */
  info->saved_regs[TLCS900_SP_REGNUM].set_value (info->prev_sp + addr_len);
  return info;
}
...
static void
tlcs900_frame_this_id (struct frame_info *this_frame, void **this_cache,
		   struct frame_id *this_id)
{
  ...
  info = tlcs900_frame_unwind_cache (this_frame, this_cache);
...
static struct value *
tlcs900_frame_prev_register (struct frame_info *this_frame,
			 void **this_prologue_cache, int regnum)
{
  struct tlcs900_unwind_cache *info
    = tlcs900_frame_unwind_cache (this_frame, this_prologue_cache);

  if (regnum == TLCS900_PC_REGNUM)
    {
      if (info->saved_regs[TLCS900_PC_REGNUM].is_addr ())
	{
	  ...
	  read_memory (info->saved_regs[TLCS900_PC_REGNUM].addr (),
...
static int
tlcs900_breakpoint_kind_from_pc (struct gdbarch *gdbarch, CORE_ADDR *pcptr)
{
  static int addr = -1;
  if (addr == -1)
    {
...
tlcs900_sw_breakpoint_from_kind (struct gdbarch *gdbarch, int kind, int *size)
{
...
tlcs900_software_single_step (struct regcache *regcache)
{
  ...
  regcache->cooked_read (TLCS900_PC_REGNUM, &addr);
  read_memory (addr, buf, sizeof(buf));
  info = tlcs900_get_insn_info (gdbarch, buf, &size);
  ret[0] = addr + size;
  ...
  switch (info->type)
    {
    case insn_djnz_d:
      regcache->cooked_read (TLCS900_BC_REGNUM, &t);
      if ((t & 0xff00) != 0x100)
	return ret;
      break;
    case insn_jr_cc_d:
    ...
    case insn_ret_cc:
      regcache->cooked_read (TLCS900_AF_REGNUM, &t);
  ...
  /* TODO: implement eTLCS900 MADL support */
  switch (info->type)
    {
    default:
      return ret;
    ...
    case insn_jp_rr:
      if (size == 1)
	opcode = TLCS900_HL_REGNUM;
      else
	opcode = (buf[size-2] & 0x20) ? TLCS900_IY_REGNUM : TLCS900_IX_REGNUM;
    ...
    case insn_ret:
    case insn_ret_cc:
      regcache->cooked_read (TLCS900_SP_REGNUM, &addr);
      read_memory (addr, buf, 3);
      addr = buf[1] * 0x100 + buf[0];
      if (gdbarch_bfd_arch_info (gdbarch)->mach == bfd_mach_etlcs900_adl)
...
static void
tlcs900_free_overlay_region_table (void)
{
...
static int
tlcs900_read_overlay_region_table ()
{
  ...
  tlcs900_free_overlay_region_table ();
...
static int
tlcs900_overlay_update_1 (struct obj_section *osect)
{
  int i;
...
static void
tlcs900_overlay_update (struct obj_section *osect)
{
  /* Always need to read the entire table anew.  */
  if (!tlcs900_read_overlay_region_table ())
    return;

  /* Were we given an osect to look up?  NULL means do all of them.  */
  if (osect != nullptr && tlcs900_overlay_update_1 (osect))
    return;
...
static int
tlcs900_insn_is_call (struct gdbarch *gdbarch, CORE_ADDR addr)
{
  gdb_byte buf[8];
  int size;
  const struct insn_info *info;
...
static int
tlcs900_insn_is_ret (struct gdbarch *gdbarch, CORE_ADDR addr)
{
  gdb_byte buf[8];
  int size;
  const struct insn_info *info;
...
static int
tlcs900_insn_is_jump (struct gdbarch *gdbarch, CORE_ADDR addr)
{
  gdb_byte buf[8];
  int size;
  const struct insn_info *info;
...
tlcs900_frame_unwind =
{
  "tlcs900",
  NORMAL_FRAME,
  default_frame_unwind_stop_reason,
  tlcs900_frame_this_id,
  tlcs900_frame_prev_register,
  NULL, /*unwind_data*/
...
/* Initialize the gdbarch struct for the TLCS900 arch */
static struct gdbarch *
tlcs900_gdbarch_init (struct gdbarch_info info, struct gdbarch_list *arches)
{
  struct gdbarch *gdbarch;
  ...
  if (tdesc_has_registers (tdesc))
    {
      ...
      feature = tdesc_find_feature (tdesc, "org.gnu.gdb.tlcs900.cpu");
      if (feature == NULL)
	return NULL;
      ...
      for (unsigned i = 0; i < TLCS900_NUM_REGS; i++)
  ...
  set_gdbarch_num_regs (gdbarch, (mach == bfd_mach_etlcs900_adl) ? ETLCS900_NUM_REGS
							     : TLCS900_NUM_REGS);
  set_gdbarch_sp_regnum (gdbarch, TLCS900_SP_REGNUM);
  set_gdbarch_pc_regnum (gdbarch, TLCS900_PC_REGNUM);

  set_gdbarch_register_name (gdbarch, tlcs900_register_name);
  set_gdbarch_register_type (gdbarch, tlcs900_register_type);
  ...
  set_gdbarch_return_value (gdbarch, tlcs900_return_value);

  set_gdbarch_skip_prologue (gdbarch, tlcs900_skip_prologue);
  set_gdbarch_inner_than (gdbarch, core_addr_lessthan); // falling stack

  set_gdbarch_software_single_step (gdbarch, tlcs900_software_single_step);
  set_gdbarch_breakpoint_kind_from_pc (gdbarch, tlcs900_breakpoint_kind_from_pc);
  set_gdbarch_sw_breakpoint_from_kind (gdbarch, tlcs900_sw_breakpoint_from_kind);
  set_gdbarch_insn_is_call (gdbarch, tlcs900_insn_is_call);
  set_gdbarch_insn_is_jump (gdbarch, tlcs900_insn_is_jump);
  set_gdbarch_insn_is_ret (gdbarch, tlcs900_insn_is_ret);

  set_gdbarch_overlay_update (gdbarch, tlcs900_overlay_update);

  frame_unwind_append_unwinder (gdbarch, &tlcs900_frame_unwind);
  if (tdesc_data)
...
{ /* table with double prefix check */
  ...
  /* initial table for eTLCS900_tlcs900 */
  { 0100, 0377, 1, insn_tlcs900      }, //eTLCS900 mode prefix
  { 0111, 0377, 1, insn_tlcs900      }, //eTLCS900 mode prefix
  { 0122, 0377, 1, insn_adl      }, //eTLCS900 mode prefix
  { 0133, 0377, 1, insn_adl      }, //eTLCS900 mode prefix
  /* here common TLCS900/Z180/eTLCS900 opcodes */
  ...
  { 0335, 0337, 1, insn_tlcs900_ddfd }, //DD/FD prefix
  { 0351, 0377, 1, insn_jp_rr    }, //"jp (hl)"
  { 0355, 0377, 1, insn_tlcs900_ed   }, //ED prefix
  { 0371, 0377, 1, insn_ld_sp_rr }, //"ld sp,hl"
  { 0000, 0000, 1, insn_default  }  //others
} ;

static const struct insn_info
etlcs900_adl_main_insn_table[] =
{ /* table with double prefix check */
  { 0100, 0377, 0, insn_force_nop}, //double prefix
  { 0111, 0377, 0, insn_force_nop}, //double prefix
  { 0122, 0377, 0, insn_force_nop}, //double prefix
  { 0133, 0377, 0, insn_force_nop}, //double prefix
  /* initial table for eTLCS900_adl */
  { 0000, 0367, 1, insn_default  }, //"nop", "ex af,af'"
  ...
  { 0030, 0377, 2, insn_jr_d     }, //"jr dis"
  { 0040, 0347, 2, insn_jr_cc_d  }, //"jr cc,dis"
  { 0100, 0377, 1, insn_tlcs900      }, //eTLCS900 mode prefix (short instruction)
  { 0111, 0377, 1, insn_tlcs900      }, //eTLCS900 mode prefix (short instruction)
  { 0122, 0377, 1, insn_adl      }, //eTLCS900 mode prefix (long instruction)
  { 0133, 0377, 1, insn_adl      }, //eTLCS900 mode prefix (long instruction)
  { 0100, 0300, 1, insn_default  }, //"ld r,r", "halt"
  { 0200, 0300, 1, insn_default  }, //"alu_op a,r"
  { 0300, 0307, 1, insn_ret_cc   }, //"ret cc"
  { 0301, 0317, 1, insn_pop_rr   }, //"pop rr"
...
*/
static const struct insn_info
etlcs900_ed_insn_table[] =
{
  /* eTLCS900 only instructions */
  ...
  /* Z180/eTLCS900 only instructions */
...
tlcs900_get_insn_info (struct gdbarch *gdbarch, const gdb_byte *buf, int *size)
{
  ...
  *size = 0;
  switch (mach)
    {
    case bfd_mach_etlcs900_tlcs900:
      info = &etlcs900_main_insn_table[4]; /* skip force_nops */
      break;
    case bfd_mach_etlcs900_adl:
      info = &etlcs900_adl_main_insn_table[4]; /* skip force_nops */
      break;
    default:
      info = &etlcs900_main_insn_table[8]; /* skip eTLCS900 prefices and force_nops */
      break;
    }
  do
    {
	...
	{
	case insn_tlcs900:
	  if (mach == bfd_mach_etlcs900_tlcs900 || mach == bfd_mach_etlcs900_adl)
	    info = &etlcs900_main_insn_table[0];
	  else
	    info = &etlcs900_main_insn_table[8];
	  break;
	case insn_adl:
	  info = &etlcs900_adl_main_insn_table[0];
	  break;
	/*  These two (for GameBoy TLCS900 & TLCS900 Next CPUs) haven't been tested.

	case bfd_mach_gbtlcs900:
	  info = &gbtlcs900_main_insn_table[0];
	  break;
	case bfd_mach_tlcs900n:
	  info = &tlcs900n_main_insn_table[0];
	  break;
	*/
	case insn_tlcs900_ddfd:
	  if (mach == bfd_mach_etlcs900_tlcs900 || mach == bfd_mach_etlcs900_adl)
	    info = &etlcs900_ddfd_insn_table[0];
	  else
	    info = &etlcs900_ddfd_insn_table[2];
	  break;
	case insn_adl_ddfd:
	  info = &etlcs900_adl_ddfd_insn_table[0];
	  break;
	case insn_tlcs900_ed:
	  info = &etlcs900_ed_insn_table[0];
	  break;
	case insn_adl_ed:
	  info = &etlcs900_adl_ed_insn_table[0];
	  break;
	case insn_force_nop:
	  return NULL;
	default:
...
extern initialize_file_ftype _initialize_tlcs900_tdep;

void
_initialize_tlcs900_tdep ()
{
  register_gdbarch_init (bfd_arch_tlcs900, tlcs900_gdbarch_init);
  initialize_tdesc_tlcs900 ();
}
